...
void init_symTable(const char * elf_file){
	...
	//读取ELF文件头
	Elf64_Ehdr ehdr;
	Assert(fread(&ehdr,sizeof(Elf64_Ehdr),1,fp) == 1, "read the elf header failed");
	...
	for(int i = 0; i < ehdr.e_shnum; ++i){
		Assert(fread(&shdr,sizeof(Elf64_Shdr),1,fp) == 1, "read the elf section header failed");
