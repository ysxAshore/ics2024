...
{
  NEMU_KEY_NONE = 0,
  MAP(NEMU_KEYS, NEMU_KEY_NAME) // NEMU_KEY_ESCAPE....
};
...
void send_key(uint8_t scancode, bool is_keydown)
{
  if (nemu_state.state == NEMU_RUNNING && keymap[scancode] != NEMU_KEY_NONE)
  {
    ...
    key_enqueue(am_scancode); // 入队列
  }
}
#else // !CONFIG_TARGET_AM
#define NEMU_KEY_NONE 0
// 未定义CONFIG_TARGET_AM时的出队函数
static uint32_t key_dequeue()
{
...
// 读写CONFIG_I8042_DATA_MMIO/CONFIG_I8042_DATA_PORT映射的寄存器的处理函数
// 只能使用read且offset必须是0
static void i8042_data_io_handler(uint32_t offset, int len, bool is_write)
{
  assert(!is_write);
  assert(offset == 0);
  i8042_data_port_base[0] = key_dequeue(); // 那么可以获得一个按键 放在映射起始地址上
}
