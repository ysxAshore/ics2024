...
word_t unsigned2signed(word_t ans)
{
  word_t ref = CONFIG_ISA64 ? 0x80000000 : 0x8000;
  if (ans >= ref)
    return ans | 0xFFFFFFFF00000000;
  else
    return ans;
}
...
void bins_excute(word_t *dnpc, word_t pc, word_t src1, word_t src2, word_t imm, int type)
{
  ...
  case 1:
    ...
    break;
  case 2:
    if ((long long)src1 < (long long)src2)
      *dnpc = branch;
    break;
  case 3:
    if ((long long)src1 >= (long long)src2)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)         \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb, I, R(rd) = unsigned2signed(Mr(src1 + imm, 1)));
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh, I, R(rd) = unsigned2signed(Mr(src1 + imm, 2)));
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw, I, R(rd) = unsigned2signed(Mr(src1 + imm, 4)));
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00110 11", addiw, I, R(rd) = unsigned2signed(src1 + imm));
  ...
  INSTPAT("0000000 ????? ????? 000 ????? 01110 11", addw, R, R(rd) = unsigned2signed(src1 + src2));
  INSTPAT("0100000 ????? ????? 000 ????? 01110 11", subw, R, R(rd) = unsigned2signed(src1 - src2));
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq, B, Bins(&s->dnpc, s->pc, src1, src2, unsigned2signed(imm), 0));
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne, B, Bins(&s->dnpc, s->pc, src1, src2, unsigned2signed(imm), 1));
