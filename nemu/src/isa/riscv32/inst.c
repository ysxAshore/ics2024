...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)         \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb, I, R(rd) = ~Mr(src1 + imm, 1) + 1);
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh, I, R(rd) = ~Mr(src1 + imm, 2) + 1);
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw, I, R(rd) = ~Mr(src1 + imm, 4) + 1);
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00110 11", addiw, I, R(rd) = ~(src1 + imm) + 1);
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb, S, Mw(src1 + imm, 1, src2));
  INSTPAT("??????? ????? ????? 011 ????? 01000 11", sd, S, Mw(src1 + imm, 8, src2));
  INSTPAT("0000000 ????? ????? 000 ????? 01110 11", addw, R, R(rd) = ~(src1 + src2) + 1);
  INSTPAT("0100000 ????? ????? 000 ????? 01110 11", subw, R, R(rd) = ~(src1 - src2) + 1);
