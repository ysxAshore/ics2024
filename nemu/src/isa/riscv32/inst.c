...
void jalr_excute(word_t *dnpc, word_t pc, int rd, word_t imm)
{
  *dnpc = (pc + imm) & ~1;
  R(rd) = pc + 4;
}
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)         \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00110 11", addi, I, R(rd) = src1 + imm);
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu, I, R(rd) = Mr(src1 + imm, 1));
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb, S, Mw(src1 + imm, 1, src2));
  INSTPAT("??????? ????? ????? 011 ????? 01000 11", sd, S, Mw(src1 + imm, 2, src2));
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal, J, Jal(&s->dnpc, s->pc, rd, imm));
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr, I, Jalr(&s->dnpc, s->pc, rd, imm));
