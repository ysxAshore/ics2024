...
#define BITMASK_128(bits) (((__uint128_t)1 << (bits)) - 1)
#define BITS_128(x, hi, lo) (((x) >> (lo)) & BITMASK_128((hi) - (lo) + 1))
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)         \
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 00110 11", srliw, I, R(rd) = SEXT(BITS(src1 >> BITS(imm, 4, 0), 31, 0), 32));
  INSTPAT("0100000 ????? ????? 101 ????? 00110 11", sraiw, I, R(rd) = SEXT(BITS((int64_t)src1 >> BITS(imm, 4, 0), 31, 0), 32));
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh, R, R(rd) = BITS_128((__int128_t)src1 * (__int128_t)src2, 127, 64));
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu, R, R(rd) = BITS_128((__int128_t)src1 * (__uint128_t)src2, 127, 64));
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu, R, R(rd) = BITS_128((__uint128_t)src1 * (__uint128_t)src2, 127, 64));
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", _div, R, R(rd) = (int64_t)src1 / (int64_t)src2);
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 01110 11", srlw, R, R(rd) = SEXT(BITS(src1 >> BITS(src2, 4, 0), 31, 0), 32));
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", sraw, R, R(rd) = SEXT(BITS((int64_t)src1 >> BITS(src2, 4, 0), 31, 0), 32));
  INSTPAT("0000001 ????? ????? 000 ????? 01110 11", mulw, R, R(rd) = SEXT(BITS(BITS(src1, 31, 0) * BITS(src2, 31, 0), 31, 0), 32));
  INSTPAT("0000001 ????? ????? 100 ????? 01110 11", divw, R, R(rd) = SEXT(BITS((int64_t)BITS(src1, 31, 0) / (int64_t)BITS(src2, 31, 0), 31, 0), 32));
  INSTPAT("0000001 ????? ????? 101 ????? 01110 11", divuw, R, R(rd) = SEXT(BITS(BITS(src1, 31, 0) / BITS(src2, 31, 0), 31, 0), 32));
  INSTPAT("0000001 ????? ????? 110 ????? 01110 11", remw, R, R(rd) = SEXT(BITS((int64_t)BITS(src1, 31, 0) % (int64_t)BITS(src2, 31, 0), 31, 0), 32));
  INSTPAT("0000001 ????? ????? 111 ????? 01110 11", remuw, R, R(rd) = SEXT(BITS(BITS(src1, 31, 0) % BITS(src2, 31, 0), 31, 0), 32));
