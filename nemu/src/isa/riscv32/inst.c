...
#define immB() do { *imm = (SEXT(BITS(i, 31, 31), 1) << 12) | (SEXT(BITS(i, 30, 25), 6) << 5) | (SEXT(BITS(i, 11, 8),4) << 1); } while(0) 
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add    , I, R(rd) = src1 + src2);
  INSTPAT("0100000 ????? ????? 000 ????? 01100 11", sub    , I, R(rd) = src1 - src2);
  INSTPAT("0000000 ????? ????? 010 ????? 01100 11", slt    , I, R(rd) = (sword_t)src1 < (sword_t)src2);
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , I, R(rd) = src1 < src2);
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , I, R(rd) = src1 < src2);
  INSTPAT("0000000 ????? ????? 000 ????? 01110 11", addw   , I, R(rd) = SEXT(BITS(src1 + src2,31,0),32));
  INSTPAT("0000000 ????? ????? 000 ????? 01110 11", subw   , I, R(rd) = SEXT(BITS(src1 - src2,31,0),32));
